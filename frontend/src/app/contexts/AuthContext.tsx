...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const setWeb3AuthUserProfile = async (user: User) => {
    try {
      ...
      if (userInfo) {
        ...
        const avatar_url = await uploadAvatarFromUrl(
          userInfo.profileImage || '/images/no-user-icon.png',
          user.id
        )
  ...
  return (
    <AuthContext.Provider
      value={{ user, login, logout, getProvider, getSepoliaBalance }}
    >
      {children}
    </AuthContext.Provider>
  )
