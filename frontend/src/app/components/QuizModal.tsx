  ...
  const handleSubmit = async () => {
    if (currentQuestionIndex < quizzes.length - 1) {
      ...
      const newScore = Object.keys(fetchedAnswers).reduce((acc, quizId) => {
        return (
          acc +
          (userAnswers[Number(quizId)] ===
          fetchedAnswers[Number(quizId)].correct_option_id
            ? 1
            : 0)
        )
      }, 0)
  ...
  return (
    <div>
      <Button
        ...
        className="bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-bold py-2 px-4 rounded-full shadow-lg transform transition duration-500 hover:scale-105"
      >
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="w-full max-w-[95vw] sm:max-w-[90vw] md:max-w-[80vw] lg:max-w-[70vw] xl:max-w-[60vw] bg-gradient-to-br from-gray-900 to-gray-800 text-white rounded-xl shadow-2xl p-4 sm:p-6">
          <DialogHeader>
            ...
            <p className="text-center text-base sm:text-lg mt-2 text-blue-300">
              全問正解でNFTをGET！
            </p>
          </DialogHeader>
                ...
                <div className="flex justify-between items-center text-xs sm:text-sm font-medium">
                  ...
                  <span>
                    {showResults
                      ? 100
                      : Math.round(
                          (currentQuestionIndex / quizzes.length) * 100
                        )}
                    % 完了
                  </span>
                </div>
                  ...
                  >
                    {!showResults ? (
                      <div className="mb-4 sm:mb-8 p-4 sm:p-6 bg-gray-800 rounded-xl shadow-inner">
                        <h3 className="text-lg sm:text-xl font-semibold mb-3 sm:mb-4 text-blue-300">
                        ...
                        <div className="space-y-2 sm:space-y-3">
                          {currentQuiz?.options.map((option) => (
                            <motion.div
                              ...
                              onClick={() =>
                                handleAnswerChange(currentQuiz.id, option.id)
                              }
                            >
                              <div
                                className={`w-4 sm:w-5 h-4 sm:h-5 rounded-full border-2 ${
                                  userAnswers[currentQuiz.id] === option.id
                      ...
                      <div className="space-y-4">
                        ...
                        {showNFTReward && <MintNFTButton location={location} />}
                        <div className="max-h-[40vh] overflow-y-auto pr-2">
                          {quizzes.map((quiz, index) => (
                            <div
                              key={quiz.id}
                              className="p-3 sm:p-4 bg-gray-800 rounded-xl shadow-inner mb-3 sm:mb-4"
                            >
                              <h3 className="text-base sm:text-lg font-semibold mb-2 text-blue-300">
                                問題 {index + 1}: {quiz.question_text}
                              </h3>
                                ...
                                {quiz.options.map((option) => (
                                  <div
                                    ...
                                    className={`p-2 rounded-lg flex items-center justify-between ${
                                      answers[quiz.id]?.correct_option_id ===
                                      option.id
                                        ? 'bg-green-600'
                              ...
                              {answers[quiz.id] && (
                                <div className="mt-2 p-2 bg-gray-700 rounded-lg text-xs sm:text-sm">
                                  <p className="font-semibold mb-1">
                                    <span
                                      className={`${userAnswers[quiz.id] === answers[quiz.id].correct_option_id ? 'text-green-400' : 'text-red-400'}`}
                                    >
                                      {userAnswers[quiz.id] ===
                                      answers[quiz.id].correct_option_id
                                        ? '正解です！'
                                  ...
                                  <a
                                    href={answers[quiz.id].additional_resources}
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className="text-blue-400 hover:underline text-xs sm:text-sm mt-1 inline-block"
                                  >
                                    詳しく学ぶ
                                  </a>
                                </div>
