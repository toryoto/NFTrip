...
export default function MintNFTButton({
  ...
  const [mintingStage, setMintingStage] = useState<
    'preparing' | 'metadata' | 'minting' | 'complete' | 'error' | null
  >(null)
  ...
  const handleMintNFT = async () => {
    ...
    try {
      ...
      NFTMetadataHash = await generateAndUploadNFTMetaData(
        imageHash,
        location,
        user
      )
      ...
      if (
        ...
        errorDescription =
          'トランザクションを拒否しました。ガス代が十分であることを確認してください。'
      } else if (
        ...
        errorTitle = 'ガス代不足'
        errorDescription =
          'ガス代が不足しています。ウォレットに十分な資金があることを確認してください。'
      } else if (
        error?.message.includes(
          'User has already minted for this location today'
        )
      ) {
        errorTitle = 'デイリーNFT Mint制限'
        errorDescription =
          'NFT Mintは1日1回までです。後日また訪問してクイズに挑戦してください。'
      }
      ...
      >
        ...
        >
          ...
          <p className="text-white text-xs sm:text-sm mb-2">
            {isMinting
              ? 'NFTを作成中...'
              : 'ここをクリックしてNFTをゲットしてください！'}
          </p>
          <p className="text-white/80 text-[10px] sm:text-xs">
            このNFTは{location.name}
            への訪問とクイズクリアの証明として永続的に保存されます
          </p>
        </div>
      </button>
    </>
