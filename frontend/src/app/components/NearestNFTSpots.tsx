      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {nearestLocations.map((location) => (
          <div key={location.id} className="group">
            <Card className="bg-gray-800 border-gray-700 overflow-hidden rounded-lg transition-all duration-300 hover:shadow-lg hover:shadow-blue-500/20 h-full flex flex-col">
              <Link href={`/spots/${location.slug}`} className="block flex-grow">
                ...
                <CardContent className="p-4 relative flex flex-col justify-between flex-grow">
                  <div>
                    <h3 className="text-xl font-semibold mb-2 text-blue-400 group-hover:text-blue-300 transition-colors duration-300 truncate">
                      {location.name}
                    </h3>
