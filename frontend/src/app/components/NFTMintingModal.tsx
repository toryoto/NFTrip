  ...
  const stages = [
    {
      ...
      icon: ImageIcon,
      educationalTip:
        'NFTは「Non-Fungible Token（非代替性トークン）」の略で、デジタルデータの唯一性を証明する技術です。あなたの観光記念は、この技術によって唯一無二の記録として残ります。'
    },
    {
      id: 'metadata',
      title: 'NFTデータの作成',
      ...
      icon: Database,
      educationalTip:
        'データには、あなたのウォレットアドレス、観光地の情報、訪問日時など、あなたの観光体験に関する重要な情報が含まれています。これらは改ざんができない形で永続的に保存されます。'
    },
    {
      id: 'minting',
      title: 'ブロックチェーンへの記録',
      description:
        'あなたの観光体験をブロックチェーン上に永続的に記録しています',
      icon: Share2,
      educationalTip:
        'ブロックチェーンは、データを分散して保存する技術です。一度記録された情報は改ざんが困難で、あなたの観光記念は永続的に証明可能な形で残ります。'
    }
  ]
          ...
          {stages.map((s, index) => {
            ...
            return (
              ...
              >
                ...
                <AnimatePresence>
                  {isCurrent && <EducationalTip tip={s.educationalTip} />}
                </AnimatePresence>
              </motion.div>
            )
          })}
